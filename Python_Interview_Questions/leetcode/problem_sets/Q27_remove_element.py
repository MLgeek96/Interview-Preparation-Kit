...
def removeElement(nums: List[int], val: int) -> int:
    ...
    • 0 <= val <= 100
    """
    assert 0 <= len(nums) <= 100, "Length of nums must be between 0 and 100"
    for num in nums:
        assert 0 <= num <= 50, "Integer in nums must be between 0 and 50"
    assert 0 <= val <= 100, "Value provided must be between 0 and 100"

    new_length = 0
    for index in range(len(nums)):
        if nums[index] != val:
            nums[new_length] = nums[index]
            new_length += 1
    return new_length